<?php
// ybrowserauth.class.php5 -- PHP5 classes for using Yahoo's browser
// authentication service
//
// Author: Jason Levitt
// March 19th, 2007 Version 1.6
// Credits: Based on code by Allen Tom with additions by Dan Theurer, Ryan Kennedy, Paul Hammond, and Zack Steinkamp
// Requirements: PHP5. The Curl extension is required.
//               For YahooMailSoap, the PHP5 SOAP extension is required
...
// YahooMailSoap..........(child of YBBauthWebServices) makes authenticated SOAP web service calls
//                        for the Mail API
...
// $authObj = new YahooMailSoap($yourappid, $yoursecret); // for Yahoo! Mail API access via SOAP
// $authObj = new YahooMailJSONRPC($yourappid, $yoursecret); // for Yahoo! Mail API access via JSON RPC
//
//
...
class YBrowserAuth {
	...
	public $appdata = null;
	public $userhash;
	public $sig_validation_error;
	const wslogin_prefix = 'https://api.login.yahoo.com/WSLogin/V1/';
	 ...
	 */
	public function __construct($yourappid, $yoursecret) {
	 ...
	 */
	public function getAuthURL($appd=null, $hash=false) {
		...
		return $this->createAuthURL(self::wslogin_prefix . "wslogin?appid=" . $this->appid . $appdata . $hashdata);
	}
	 ...
	 */
	protected function createAuthURL($url) {
		// Take apart the URL
		$parts = parse_url($url);
	 ...
	 */
	public function validate_sig($ts=null, $sig=null) {
...
class YBBauthWebServices extends YBrowserAuth {
	...
	public $WSSID;
	public $cookie;
	public $access_credentials_error;
	public $web_services_error;
	...
	protected function getAccessURL() {
		...
		return $this->createAuthURL(parent::wslogin_prefix . "wspwtoken_login?token=$this->token&appid=$this->appid");
	}
...
class YBBauthREST Extends YBBauthWebServices {
	 ...
	 */
	public function createAuthWSurl($url) {
     ...
     * in this method.
	 */
	public function makeAuthWSgetCall($url) {
     ...
     * in this method.
	 */
	public function makeAuthWSpostCall($url) {
...
class YahooMailSoap Extends YBBauthWebServices {
	const wsdl_path = 'http://mail.yahooapis.com/ws/mail/v1.1/wsdl';
	const mail_prefix = 'http://mail.yahooapis.com/ws/mail/v1.1/soap';
...
class YahooMailJSONRPC Extends YBBauthWebServices {
	const jsonrpc_endpoint = 'http://mail.yahooapis.com/ws/mail/v1.1/jsonrpc';
